 ...
 *
 * NOTE: this script is modified for use with this application, you can see the modifications by searching for "Webiny"
 *
 */
(function ($) {
	function _1(_2) {
	...
	function _4(_5) {
		...
		$(_5).unbind().bind("mouseover", function (e) {
			...
			_e9(_5, _a[0]);
			// Old code: _6.onDblClick.call(_5, _c(_5, _a[0]));
			// Webiny
			_6.onDblClick.call(_5, _c(_5, _a[0]), e);
			// /Webiny
			e.stopPropagation();
	...
	function _a4(_a5, _a6) {
		...
		if (_a7.length == 0) {
			...
			// Webiny
			// Old code: if (_56(_a5, _a7[0])) {
			var targetNode = _c($.data(_a5, 'tree').tree[0], _a7);
			if(_56(_a5, _a7[0])){
				// /Webiny
				var _a9 = _a7.find("span.tree-icon");
				_a9.removeClass("tree-file").addClass("tree-folder tree-folder-open");
				targetNode.type = 'tree-folder';
	...
	function _b0(_b1, _b2) {
		...
		if (_b3) {
			if (!_b3.children || !_b3.children.length) {
				var _b4 = $(_b3.target);
				// Webiny
				// Old code: _b4.find(".tree-icon").removeClass("tree-folder").addClass("tree-file");
				var tree = $.data(_b1, "tree").options;
				tree.onFolderEmpty(_b3);
				// /Webiny
	...
	function _f7(_f8, _f9) {
		...
		// Webiny
		_fb.parent().removeClass('editable');
		// /Webiny
		var val = _fb.val();
		_fb.remove();
		var _fc = _c(_f8, _f9);
		...
		render: function (_128, ul, data) {
			...
			function _12a(_12b, _12c) {
				...
				for (var i = 0; i < _12c.length; i++) {
					var item = _12c[i];
					// Webiny
					opts.onBeforeItemRender.call(tree, item);
					// /Webiny
					...
					if (item.state == "closed") {
						...
						if (item.children && item.children.length) {
							...
							cc.push("<span class=\"tree-icon " + type + " " + (item.iconCls ? item.iconCls : "") + "\"></span>");
							// /Webiny
							_12d = true;
						}
					}
